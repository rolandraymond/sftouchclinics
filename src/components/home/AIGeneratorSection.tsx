...
import { Sparkles, UploadCloud, Loader2, ChevronsLeftRight, AlertCircle } from 'lucide-react';
...
const AIGeneratorSection = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setError(isRTL ? 'حجم الصورة كبير جداً. الحد الأقصى 4 ميجا' : 'Image is too large. Max 4MB.');
      return;
    }
  ...
  console.error(err); // لمشاهدة تفاصيل الخطأ في الـ Console إن أردت
      setError(isRTL ? 'عذراً، فشل توليد الصورة. حاول مرة أخرى.' : 'Failed to generate image. Try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section dir={isRTL ? 'rtl' : 'ltr'} className="py-24 bg-slate-950 relative overflow-hidden">
      {/* Background Glow */}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-12 shadow-2xl">
          ...
          {!originalImage && (
            ...
            >
              ...
              <p className="text-slate-500 text-sm">
                {isRTL ? 'يفضل صورة واضحة للوجه بإضاءة جيدة (أقل من 4MB)' : 'Clear face photo with good lighting (Max 4MB)'}
              </p>
            </div>
          )}
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
              ...
              <h3 className="text-lg font-bold text-amber-400 animate-pulse">
                {isRTL ? 'جاري تحليل ملامح الوجه وتطبيق اللمسات السحرية...' : 'Analyzing facial features & applying magic...'}
              </h3>
            </div>
          )}
          ...
          {!loading && originalImage && aiImage && (
            <div className="relative w-full max-w-2xl mx-auto h-[400px] md:h-[500px] rounded-2xl overflow-hidden shadow-2xl select-none">
              ...
              {/* After Image (AI) */}
              <img src={aiImage} alt="After" className="absolute inset-0 w-full h-full object-cover" />
              ...
              >
                <img src={originalImage} alt="Before" className="absolute inset-0 w-full h-full object-cover" />
              </div>
          ...
          {!loading && originalImage && !aiImage && (
            <div className="relative w-full max-w-md mx-auto rounded-2xl overflow-hidden shadow-xl border border-slate-700">
              <img src={originalImage} alt="Uploaded" className="w-full h-auto" />
            </div>
          )}
